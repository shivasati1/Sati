...
const COLLECTION = "takerBuySellRatio";
...
async function fetchTakerBuySellRatio(symbol) {
  ...
  return parseFloat(res.data[0].buySellRatio);
}
...
(async () => {
  try {
    ...
    for (const symbol of symbols) {
      ...
      const ratio = await fetchTakerBuySellRatio(symbol);
      if (ratio === null) {
        console.log(`No taker buy/sell ratio for ${symbol}`);
        continue;
      }
      ...
      console.log(`Saved taker buy/sell ratio for ${symbol}: ${ratio}`);
